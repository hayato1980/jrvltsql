...
- **`fetch --from A --to B` が `JVOpen` へ渡す `fromtime` は 2 通りになった。**

  | spec | option | 渡る `fromtime` | サーバから降ってくる量 |
  |---|:-:|---|---|
  | `RACE` / `SLOP` / `WOOD` | 1 / 3 / 4 | `A000000-B235959`（ハイフンで繋ぐ） | A〜B の窓のぶんだけ |
  | それ以外の全 spec | 1 / 3 / 4 | `A000000`（繋がない） | A 以降ぜんぶ |
  | すべての spec | 2 | `A000000`（繋がない） | 現在の開催サイクル |

  従来は常に `A000000` で、`--to` は取得後のクライアント側フィルタでしかなかった。
  そのため `option=3/4` のセットアップが実行当日までを全量ダウンロードしていた
  （実測 78 GB でディスクが満杯になった）。`RACE`・2022 年窓の実測で、繋ぐと 167 件、
  繋がないと 876 件。**繋いでもクライアント側フィルタは引き続き働く**
  （窓の外の日付を含むファイルが届くため）
- 繋ぐ spec は `RANGE_FROMTIME_DATA_SPECS` の**許可リスト**（実測済みのものだけ）で持つ。
  日付フィールドを持たないコード主キーのマスタ系（`DIFN` / `BLDN`）は、繋ぐと JV-Link が
  **エラーを返さずに 0 件**を返すため（2022 年窓・2025 年窓で実測。繋がなければ 221,917 件）。
  拒否リストにすると新しい dataspec を足すたびに無言の 0 件を踏む側が既定になる。
  繋がない側は繋ぐ側の上位集合なので、未実測を繋がない側へ倒す代償はダウンロード量であって
  取りこぼしではない
- **CLI フラグは増やしていない。** 繋ぐかどうかは spec から決まる性質で、呼び出し側が
  指定するものではない。`--to` の help と実行時 Note は、渡る `fromtime` の実値を
  そのまま見せる形に改めた
