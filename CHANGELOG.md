...
- `option 4`（分割セットアップ）の取り込みを、レコードの年が変わるたびにコミットするように変更。
  `JVOpen` の呼び出しは従来どおり1回のみで、対話ダイアログの回数は増えない。数十年ぶんを1つの
  トランザクションで取り込んでいたため、途中で中断すると1行も残らなかった。コミットのたびに
  標準出力へ `[commit] year=YYYY` を出力する。日付を持たないレコード（マスタ系 spec）は年境界を
  作らないため、従来どおり最後に1回だけコミットされる。レコード拒否が起きた年はロールバックされ、
  それ以前の年は残る。`option 1` / `2` は変更なし
