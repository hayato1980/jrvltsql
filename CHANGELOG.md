...
- `JVOpen` の読み出し終了ポイント時刻を `jltsql fetch --fromtime-end` で指定
  できるようにした。公式の `fromtime` は「開始のみ」と「開始-終了」の2形式が
  あり（仕様書 4.9.0.1 p.17-18）、終了ポイントを付けるとサーバへ問い合わせる
  ファイル一覧とダウンロード対象自体が絞られる。p.17「既知の障害について」が
  挙げる「対象ファイル数が多い場合に `JVRead` が遅くなる」障害の公式回避策
  （終了ポイント指定、および `--spec` の個別指定）を使えるようにするもの
- 読み出しポイントは提供時刻（ファイルタイムスタンプ）であって開催日ではない
  ため、`--to`（開催日のclient-side filter）からは自動生成しない。指定した
  取得は要求範囲を満たすとは限らないので、NLキャッシュの完全マーカーも作らない
- `TOKU`/`DIFN`/`HOSN`/`HOYU`/`COMM`（旧 `DIFF`/`HOSE`）への終了ポイント指定を
  送信前に拒否する。仕様書 p.18 のとおり JV-Link はエラーではなく `-1`
  （該当データなし）を返すため、そのままでは取得が静かに空になる。option 3/4 も
  同様に拒否する。p.20 上、終了時刻が制限するのは今月の通常データ部分だけで、
  前月までのセットアップ用データは `fromtime` より大きい全件が対象のため
- `fromtime` が公式の2形式でない場合（桁数不正・非暦日・ハイフン過多・開始が
  終了以降）も、COM/bridgeへ送る前に拒否する（従来は -112/-113 として返っていた）
