...
- `fetch --to` を `JVOpen` の `fromtime` の終了境界としても渡すように変更。
  従来 `--to` は取得後のクライアント側フィルタでしかなく、`option=3/4` の
  セットアップは指定時刻から実行当日までを全量ダウンロードしていた（実測 78 GB で
  ディスクが満杯になった）。範囲形式 `YYYYMMDDhhmmss-YYYYMMDDhhmmss` は
  ダウンロード量そのものを窓に閉じる（`RACE`・2022 年窓の実測で、範囲形式 167 件に対し
  開始のみ 876 件）。クライアント側フィルタは引き続き必要（窓の外の日付を含むファイルが
  届くため）
- 範囲形式を使う dataspec は `RANGE_FROMTIME_DATA_SPECS`（`RACE` / `SLOP` / `WOOD`）の
  **許可リスト**で持ち、それ以外は開始のみの `fromtime` へ自動でフォールバックする。
  日付フィールドを持たないコード主キーのマスタ系（`DIFN` / `BLDN`）は、範囲形式だと
  JV-Link が **エラーを返さずに 0 件** を返すため（2022 年窓・2025 年窓で実測、同じ窓を
  開始のみで開くと 221,917 件）。拒否リストにすると、新しい dataspec を足すたびに
  無言の 0 件を踏む側が既定になる。開始のみは同じ窓の範囲形式の上位集合なので、
  未実測の dataspec を開始のみへ倒す代償はダウンロード量であって取りこぼしではない。
  **CLI フラグは増やしていない**: 範囲形式を使うかどうかは dataspec から決まる性質で、
  呼び出し側が指定するものではない
- `option=2`（今週データ）は従来どおり常に開始のみ。この `fromtime` は現在の開催
  サイクル内の連続性管理に使われるもので、過去の窓を選ぶものではない
- 上記に合わせて `--to` の help と実行時 Note を「クライアント側フィルタ」から
  「終了境界（種別によっては開始のみへフォールバック）」へ改めた
