...
def test_option_4_undated_records_join_the_year_in_progress(capsys):
    # Undated records (the master data specs) carry no year of their own, so
    # they are committed with the year around them. Ones that arrive before any
    # dated record have no year to join and are committed on their own, without
    # a marker -- no year has been proven complete at that point.
    processor = _year_boundary_processor(
        [
            {"RecordSpec": "UM", "KettoNum": "1986100001"},
            _dated_record(1986),
...
def test_option_4_fetch_rejection_fails_the_run_and_leaves_committed_years(capsys):
    # The fetcher counts its rejections cumulatively over the whole stream, so
    # they are only settled once it ends -- unlike an import rejection, which
    # is known before its own year commits. The run fails and the years already
    # committed stand: the operator re-runs, and upsert makes the re-fetch
    # harmless.
    processor = _year_boundary_processor(
