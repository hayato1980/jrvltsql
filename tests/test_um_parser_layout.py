...
CHAKU = ["".join(f"{group * 6 + rank:03d}" for rank in range(1, 7)) for group in range(27)]
...
def test_um_standard_extra_unique_is_rejected_before_mutation(tmp_path, auto_commit):
    primary = SQLiteDatabase({"path": str(tmp_path / "unique.db")})
    with primary:
...
def test_um_standard_dual_rejects_extra_unique_on_either_backend(tmp_path, unsafe_target):
...
def test_um_standard_invalid_official_key_is_rejected_before_mutation(tmp_path, invalid_key):
    ...
    with database:
        ...
        with pytest.raises(SchemaMigrationError, match="KettoNum"):
            _import_standard_um_records(database, [record], True)
...
def test_um_native_storage_keeps_distinct_keys_and_updates_exact_key(tmp_path):
    database = SQLiteDatabase({"path": str(tmp_path / f"native-{DataImporter.__name__}.db")})
    with database:
...
@pytest.mark.parametrize("auto_commit", (True, False), ids=("owned", "caller-owned"))
def test_um_native_extra_unique_is_rejected_before_mutation(tmp_path, auto_commit):
    database = SQLiteDatabase({"path": str(tmp_path / "native-unique.db")})
    with database:
...
def test_um_native_missing_table_is_rejected_before_mutation(tmp_path):
    database = SQLiteDatabase({"path": str(tmp_path / "native-missing.db")})
    with database:
        with pytest.raises(SchemaMigrationError, match="Required table does not exist: NL_UM"):
            _import_native_um_records(database, [parsed_record()], True)
        assert (
...
def test_um_native_wrong_or_missing_key_is_rejected_before_mutation(tmp_path, defect):
    database = SQLiteDatabase({"path": str(tmp_path / f"native-{defect}.db")})
    with database:
...
def test_um_native_postgresql_constraint_drift_is_rejected_before_mutation(
    postgresql_db, auto_commit, replacement, message
):
    ...
    postgresql_db.rollback()
    with pytest.raises(SchemaMigrationError, match=message):
        _import_native_um_records(postgresql_db, [parsed_record()], auto_commit)
...
def test_um_native_postgresql_missing_table_is_rejected_before_mutation(postgresql_db):
    with pytest.raises(SchemaMigrationError, match="Required table does not exist: NL_UM"):
        _import_native_um_records(postgresql_db, [parsed_record()], True)
    assert postgresql_db.has_pending_transaction() is False
    assert postgresql_db.fetch_one("SELECT to_regclass('nl_um') AS oid")["oid"] is None


def test_um_native_postgresql_key_roundtrip(postgresql_db):
    postgresql_db.execute(SCHEMAS["NL_UM"])
    postgresql_db.commit()
    importer = DataImporter(postgresql_db)
