...
def test_jg_data_importer_one_record_uses_the_exact_delete_contract(
    tmp_path,
    table_name: str,
    standard: bool,
    auto_commit: bool,
...
def test_jg_postgresql_native_and_standard_revote_update_delete(postgresql_db) -> None:
    ...
    for table_name, standard in (("NL_JG", False), ("JOGAIBA", True)):
        ...
        assert postgresql_db.fetch_one(f"SELECT COUNT(*) AS count FROM {table_name}")["count"] == 2
        ...
        assert postgresql_db.fetch_one(f"SELECT COUNT(*) AS count FROM {table_name}")["count"] == 1
...
@pytest.mark.parametrize(
    ("table_name", "standard"),
    (("NL_JG", False), ("JOGAIBA", True)),
)
def test_jg_storage_rejects_an_extra_unique_before_it_can_erase_another_key(
    tmp_path,
    table_name: str,
    standard: bool,
) -> None:
        ...
        with database:
            ...
            with pytest.raises(SchemaMigrationError, match="UNIQUE"):
                importer.import_records(
                    iter([parsed_record(num="001", ketto_num="2026100001")]),
                    auto_commit=auto_commit,
                )
    ...
    with database:
        database.create_table(table_name, schema)
        importer = DataImporter(database, use_jravan_schema=standard)
        importer.import_records(iter([parsed_record(num="001", ketto_num="2026100001")]))
        importer.import_records(iter([parsed_record(num="002", ketto_num="2026100002")]))
