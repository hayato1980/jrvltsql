...
- `MakeDate` は 8 桁 ASCII 数字であることだけを要求し、日付としての妥当性は
  問いません。公式の未設定値 `00000000` も、暦に無い `20040231` のような値も、
  提供値として 8 文字のまま保持します。`MakeDate` は provenance であり、
  `TEXT` 列として等値比較にしか使わない（日付として解釈する経路が無い）ため、
  暦検証は下流の誰も必要としない棄却を生むだけだからです。8 桁 ASCII 数字の
  要求は残してあり、この span の読み出し破損は引き続き検出します。
