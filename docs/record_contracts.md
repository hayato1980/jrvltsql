...
JV-Data は「未計測」「不明」「該当なし」を、列の中の特別な値（センチネル）で表します。
jrvltsql はこれを 2 つの機構で扱い、適用範囲が異なります。

| 機構 | 対象 | 何をするか |
| --- | --- | --- |
| インポート共通の型変換 | 全テーブル | `***` / `----` / `0103*****` のような**非数値**センチネルを、数値列（`INTEGER` / `BIGINT` / `REAL`）では NULL にします。テキスト列では前後の空白を落として残します（空白だけの値は NULL です） |
| canonical 化 | SE だけ | 増減不明の `999` のような**数値**センチネルを、解釈済み列でだけ NULL にします |

数値センチネルは数値列へそのまま入るため、型変換では落ちません。落とすのは
canonical 化を持つレコード種別だけで、現状 SE だけです。SE は 1 つの提供値を
次の 3 列で保持します。

- **生値列**（`Provider*Raw`、TEXT）: 提供された文字列をそのまま保持します。
  数値センチネルも非数値センチネルも残ります。提供が空欄のときだけ NULL です。
- **そのままの数値列**（`BaTaijyu` / `ZogenSa` など公式項目名の列）: 上の型変換を
  通しただけの値です。数値センチネルは NULL にならず、数値として残ります。ただし
  公式が 0.1 単位で持つ列（`Futan` / `Odds` / `Time` / `HaronTime*` / `TimeDiff` /
  `DMTime` / `DMGosa*` など）は型変換が 1/10 するため、`999` はこれらの列では
  `99.9` として残ります。
- **解釈済み列**（`ZogenSaKg` / `FutanKg` / `HaronTimeL3Seconds` など、単位を名前に
  持つ列）: 単位が確定した値で、数値センチネルは NULL です。

`ParserContractVersion` を持つのも SE だけで、この 3 列契約を持つレコード種別の
印になります。他のレコード種別は、そのままの数値列だけを持ちます。**その列に
入っている数値センチネルを、実測値と列だけで見分けることはできません。**
...
- 馬体重と増減差は、提供がある場合は 3 桁の数字だけを受け付けます。桁数か文字種が
  外れている馬が 1 頭でもあれば、その馬だけでなくレコード全体を拒否します。予備値の
  `BaTaijyu=001` も同じくレコード全体を拒否します。提供が空欄の馬（初出走など）は
  受け付け、その馬の行では NULL として保存します。
- WH は生値列（`Provider*Raw`）も解釈済み列も持たず、`ParserContractVersion` も
  持ちません。増減不明の `999` のような数値センチネルは、そのままの数値列 `ZogenSa`
  へ数値として入ります（[センチネル値（sentinel）の扱い](#sentinel)）。増減差の符号を
  数値へ織り込んだ列も無く、符号は `ZogenFugo` が文字として持ちます。
...
- 不成立・特払・返還の 3 つの券種 flag 配列は、6 件目が公式予備です。公式が
  値を規定しないため flag として読まず、1 バイト区画に収まる限り内容を問わず
  文字列で保持します。返還馬番 28 件、返還枠番 8 件、返還同枠 8 件は全要素が
...
- `MakeDate` は実在する `YYYYMMDD`、または公式の未設定値である厳密な
  `00000000` だけを受け付けます。`20040231` のような暦に無い非ゼロ値は、
  provider provenance の破損として mutation 前に拒否します。
- native と標準名モードのどちらでも `MakeDate` を 8 文字のまま保持します。
  標準 `UMA_RACE` も `VARCHAR(8)` を使うため、PostgreSQL で `00000000` を
  `DATE` へ変換して失敗したり、欠損へ置換したりしません。
- 中止（`DataKubun=9`）そのものは `MakeDate` 未設定の根拠になりません。中止でも
  実在日を持つレコードのほうが一般的で、`00000000` は提供元が作成日を持たない
  ときの表現です。
- 馬体重と増減差は、生値・そのままの数値・解釈済みの 3 列で保持します
  （[センチネル値（sentinel）の扱い](#sentinel)）。508kg / +3kg は、生値列
  `ProviderBaTaijyuRaw` / `ProviderZogenFugoRaw` / `ProviderZogenSaRaw` へ
  `508` / `+` / `003`、そのままの数値列 `BaTaijyu` / `ZogenSa` へ 508 / 3、
  解釈済み列 `BaTaijyuKg` / `ZogenSaKg` へ kg の整数 508 / 3 を保存します。
- 増減差の符号は `ZogenFugo`（生値列は `ProviderZogenFugoRaw`）が文字として持ち、
  `ZogenSa` は符号を持たない数値です。符号を数値へ織り込むのは解釈済み列だけで、
  −3kg は `ZogenSa` が 3、`ZogenSaKg` が −3 になります。
- 増減不明の `999` のような数値センチネルは、生値列とそのままの数値列には
  そのまま残り、解釈済み列だけが NULL になります。初出走などで提供が空欄の
  場合は 3 列とも NULL です。
- 解釈済み列と `ParserContractVersion` を持つのは SE だけです。
...
- 各フラグは未設定時の公式初期値も `0` なので、非削除レコードでは常に `0` か
  `1` です。予備領域 `Yobi1`/`Yobi2` は公式が値を規定しないため、2/6 バイトの
  各区画に収まる限り内容を問わずそのまま保持します。
...
- `DataKubun=9` のレース中止 snapshot では、提供済みの組番を残したまま票数欄を
  11 バイトの空白で返す場合があります。この場合に限り、parser が正規化した
  `Hyo=""` を受け付け、数値票数列には SQL `NULL` として保存します。
  `DataKubun=2/4/5` の同じ空値、`None`、空白を残した caller 値、および公式の
  11 spaces 以外の空白バイト列は受け付けません。
...
- 現行 JV-Data 4.9.0.1 の 102,890 バイト配置（3連単 4,896 組 × 21 バイトと、
  11 バイト × 2 個の票数合計）だけを受け付けます。
- 1 レコードは 1 レースの完全な snapshot です。parser は 1 組合せを 1 行へ
  展開し、各行に header と票数合計を持たせます。
- 公式に 2004 年 8 月 14 日以降の提供で、発売のないレースにはレコードが
  提供されません。

**identity（主キー）:**

- 公式 identity はレースキー（`Year`・`MonthDay`・`JyoCD`・`Kaiji`・`Nichiji`・
  `RaceNum`）で、native では組番（`SanrentanKumi`）を加えた 7 列です。
- 標準名では header table `HYOSU2` がレースキー、子 table `HYOSU_SANRENTAN` は
  レースキー＋`Kumi` です。

**保存先:**

- native `NL_H6`、速報 `RT_H6`、標準名 `HYOSU2` / `HYOSU_SANRENTAN` はいずれも
  公式 field capacity と NOT NULL の key 列を使い、1 レース分の snapshot を
  丸ごと置換します。
- H6 は蓄積系（`RACE`）と速報系（`0B20`）の両方で提供されるため、速報経路は
  `RT_H6` へルーティングします。
...
- 人気順は数値ではありません。公式に `----`（発売前取消）・`****`（発売後取消）・
  空白（登録なし）も取り得るため、人気順列は文字列として保持します。対象は
  native `NL_H6`/`RT_H6` の `SanrentanNinki`（旧 schema は `INTEGER`）と
  標準名 `HYOSU_SANRENTAN` の `Ninki`（旧 schema は `SMALLINT`）で、旧 schema は
  取消マーカーを `NULL` に落とします。H1 の 2 文字表記（`--`/`**`）とは異なり、
  H6 の取消マーカーは 4 文字固定です。
- 空白の人気順は `NULL` ではなく空文字で保持します（他の table の
  「空欄→`NULL`」規則の例外）。
- 組番が 1 件も無い snapshot（発売フラグ `0`/`1` や `DataKubun=9` のレース中止）
  でも公式の票数合計は提供されるため、native では H1 の総計行と同じ
  `SanrentanKumi=TOTAL` の 1 行として保持します（組番票数と人気順は持ちません）。
  標準名では header table `HYOSU2` に合計が入り、子 table の行は 0 件です。
- 返還馬番情報は馬番 01〜18 の位置ごとの `0`/`1` フラグ列（18 桁）で、公式
  初期値は `0` です。provider が未設定位置を空白で送る実測があるため空白の位置も
  受け付けますが、それ以外の文字と桁落ちは拒否します。空白位置は H1 と同じく
  `NULL` として保存します（値のある位置は保持します）。
- 票数・票数合計は 11 バイト ASCII 数字（単位百円、ALL0 は発売前取消し・発売
  票数なし）です。provider が合計エリアを空白で送る場合だけ空値を許容します。
- `DataKubun=9` のレース中止 snapshot では、提供済みの組番を残したまま票数欄を
  11 バイトの空白で返す場合があります。この場合に限り、parser が正規化した
  `SanrentanHyo=""` を受け付け、数値票数列には SQL `NULL` として保存します。
  `DataKubun=2/4/5` の同じ空値、`None`、空白を残した caller 値、および公式の
  11 spaces 以外の空白バイト列は受け付けません。登録済み組番の物理票数欄は
  11 桁すべてが ASCII 数字、または status 9 の 11 spaces のどちらかでなければ
  parser で拒否します。速報の raw・parsed-record・batch 入口も同じ検証を DML
  より前に適用します。
- 発売フラグは `0`・`1`・`3`・`7` だけを受け付けます。

**既存 DB からの移行手順:**

- 既存の nullable key、主キーの欠落・短縮、型違い、容量不足、generated /
  identity 列、追加列、追加 UNIQUE/FK/CHECK を持つ `NL_H6` / `RT_H6` /
  `HYOSU2` / `HYOSU_SANRENTAN` は自動修復せず、mutation 前に停止します。
- 標準名では、公式キー以外の UNIQUE index も拒否します（別レースの行を置換や
  衝突で失うため）。import が作成する公式キー index だけを許可します。
- 既存 DB に対する追加 migration も、`NL_H6` / `RT_H6` / `HYOSU2`（子 table を
  含む）が現行契約を満たすまで停止します。
- DB をバックアップして対象 table を現行 schema で再作成し、保持中の `RACE`
  source から再取込してください。人気順列が数値型だった DB では、再取込しない
  限り取消マーカーは復元されません。

### オッズ系（O1 / O2 / O3 / O4 / O5 / O6）

**公式レイアウト:**

- 現行 JV-Data 4.9.0.1 の固定長だけを受け付けます。`O1` 962 / `O2` 2,042 /
  `O3` 2,654 / `O4` 4,031 / `O5` 12,293 / `O6` 83,285 バイトです。
- 1 レコードは 1 レース 1 時点の完全な snapshot です。parser は 1 組合せを
  1 行へ展開します。オッズ部の繰返しは公式どおり `O1` 単勝 28×8・複勝 28×12・
  枠連 36×9、`O2` 153×13、`O3` 153×17、`O4` 306×13、`O5` 816×15、
  `O6` 4,896×17 です。
- 票数合計はオッズ部の後に 11 バイトで提供されます（`O1` は単勝・複勝・枠連の
  3 個、`O2`〜`O6` は 1 個）。
- 発売のないレースについてはレコードが提供されません（公式特記事項）。

**DataKubun:**

- 公式値は `0`（削除）・`1`（中間）・`2`（前日売最終）・`3`（最終）・
  `4`（確定）・`5`（確定・月曜）・`9`（レース中止）です。
- `DataKubun=0` は本文もヘッダも持たない（provider が空白で送る）ため、
  レースキーだけを検証します。物理 exact erase はレースキーで行います。

**値の扱い:**

- オッズと人気順は数値だけではありません。公式に無投票（`0` の並び）・
  発売前取消（`-` の並び）・発売後取消（`*` の並び）・登録なし（空白）が
  あります。桁数は項目ごとに異なるため（例: `O2` オッズ 6 桁と人気順 3 桁、
  `O6` オッズ 7 桁と人気順 4 桁）、桁落ちした記号値は拒否します。
- 無投票・取消・登録なしのオッズを持つ組合せも公式の提供値です。parser は
  組番を持つ行を捨てません。組番を持たない（空白の）スロットだけを行にしません。
- 発売フラグは `0`（発売なし）・`1`（発売前取消）・`3`（発売後取消）・
  `7`（発売あり）だけを受け付けます。`O1` の複勝着払キーは `0`・`2`・`3` です。
- `HassoTime` は公式の「発表月日時分」（`MMDDhhmm`）で、中間オッズのみ設定
  されます。発走時刻ではありません。空白も提供値として許容します。
- 組合せを 1 件も持たない snapshot（レース中止・削除など）でも公式の票数合計は
  提供されるため、H1 / H6 と同じ合計行 sentinel（`Kumi=TOTAL`）で 1 行だけ
...
- 公式が項目を置かない予備領域（`reserved` 8 バイトと、公式レイアウトが現在も
  予備と記す `DelKubun` 1 バイト）は、値を解釈せず各区画に収まる限りそのまま
  保持します。
