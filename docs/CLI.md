...
`option 4` は `JVOpen` を1回だけ呼んで長期間を一括取得するため、**レコードの年が変わるたびに
コミット**します。コミットのたびに標準出力へ `[commit] year=YYYY` を出力するので、途中で中断
してもどの年まで DB に入ったかをログから読み取れます。日付を持たないレコード（マスタ系の spec）は
年境界を作らないため、従来どおり最後に1回だけコミットされます。`option 1` / `2` の差分取得は
取得全体で1トランザクションのままです。
