...
### 読み出し開始/終了ポイント（`--fromtime-end`）

```bat
jltsql fetch --from 20260101 --to 20260417 --spec RACE --option 1 --fromtime-end 20260417
```

`JVOpen` の `fromtime` は「開始ポイントのみ」と「開始ポイント-終了ポイント」の
2形式があります（仕様書 4.9.0.1 p.17-18）。`--fromtime-end` を指定すると後者の
形式で要求し、JV-Linkがサーバへ問い合わせるファイル一覧・ダウンロード対象自体が
その時刻までに絞られます。仕様書 p.17「既知の障害について」が挙げる
「対象ファイル数が多い場合に `JVRead` の処理時間が遅くなる」障害の公式回避策は
この終了ポイント指定と、`--spec` を連結せず1種別ずつ指定することの2つです。

指定時の注意:

- 開始/終了ポイントは**データの提供時刻（ファイルタイムスタンプ）**であって
  開催日ではありません。`--to` は取得後のclient-side filter（開催日）なので、
  `--to` から終了ポイントを自動生成することはしません。指定した時刻より後に
  提供されるデータ（開催後に届く成績など）は取得されません。
- 範囲を満たしたと断定できないため、`--fromtime-end` を指定した取得では
  NLキャッシュの完全マーカーを作りません。
- `TOKU` / `DIFN` / `HOSN` / `HOYU` / `COMM`（旧 `DIFF` / `HOSE`）は全データを
  取得する種別のため終了ポイントを指定できません。指定すると `JVOpen` は
  エラーではなく `-1`（該当データなし）を返して静かに空になるので、`jltsql` は
  送信前に理由を出して停止します（仕様書 p.18）。
- `option 3/4` では指定できません。p.20 のとおり終了時刻が制限するのは今月の
  通常データ部分だけで、前月までのセットアップ用データは `fromtime` より大きい
  全件が対象のためです。
...
`--spec RACEDIFN` のように連結できます。ただし連結指定は上記の既知の障害の
条件そのものなので、取得が遅い場合は1種別ずつ実行してください。連結した
dataspecに終了ポイント禁止IDが1つでも含まれると、要求全体で終了ポイントを
指定できません。
