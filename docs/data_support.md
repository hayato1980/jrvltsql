...
| データ種別 | 廃止された旧名 | 内容 | 主なレコード種別 | 保存先テーブル | option 1 | option 2 | option 3/4 | 備考 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
### 2023-08 に廃止された旧 dataspec

`DIFF` / `BLOD` / `SNAP` / `HOSE` / `TCOV` / `RCOV` は受け付けません。`fetch` /
`cache build` は取り込みに入る前に、新しい種別名を示して停止します。
...
| `0B51` | 速報重勝式 WIN5 | `WF` | `RT_WF` | `YYYYMMDD` または WIN5 開催キー | 公式7,215-byte形式（有効票数5件・払戻243件）に対応 |
