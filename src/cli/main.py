...
def _print_fetch_guardrail_notes(jv_option: int, data_specs) -> None:
    ...
    刻む dataspec と刻まない dataspec を一度に要求できるので、注記は要求全体を
    見て出す。両方混ざっていれば両方出す（どちらか一方だけだと、片側の
    dataspec について嘘になる）。
    """
    if jv_option in (3, 4):
    ...
    chunked = [s for s in data_specs if uses_range_fromtime(s, jv_option)]
    if chunked:
        err_console.print(f"[yellow]Note:[/yellow] {FETCH_NOTE_TO_RANGE_CHUNKED}")
    if len(chunked) < len(data_specs):
