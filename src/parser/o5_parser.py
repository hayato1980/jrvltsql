...
class O5Parser(odds_domain.OddsCombinationValidationMixin):
    """
    O5レコードパーサー
    ...
    SALE_FLAG_FIELDS = ("SanrenpukuFlag",)
    COMBINATION_WIDTH = 6
    ODDS_WIDTHS = (("Odds", 6),)
    FAVOURITE_WIDTH = 3
    TOTAL_FIELDS = ("Vote",)
    ...
    def parse(self, data: bytes) -> Optional[List[Dict[str, object]]]:
        ...
        try:
            ...
            base["Vote"] = vote
            rows = []
            for pos in range(40, max(40, len(data) - 13), 15):
                item = data[pos:pos + 15]
                if len(item) < 15:
                    continue
                kumi = self.decode_field(item[0:6])
                odds = self.decode_field(item[6:12])
                ninki = self.decode_field(item[12:15])
                if not kumi.strip("0 "):
                    continue
