...
class BaseParser(ABC):
    ...
    def parse(self, record: bytes) -> Dict[str, Any]:
        ...
        # Verify record type. Only the 2-byte record spec is decoded here; the
        # record as a whole stays bytes so that fields are sliced at the byte
        # offsets the JV-Data layouts are defined in.
    ...
    def _extract_field(self, record: bytes, field_def: FieldDef) -> Any:
        ...
        The record is sliced as bytes and only the field itself is decoded.
        Decoding the whole record first and slicing the resulting string would
        use character offsets, which shifts every field that follows a
        multibyte (cp932) value.
        ...
        # errors='replace' keeps an undecodable field from failing the whole
        # record; invalid sequences become '�'.
        raw_value = record[field_def.start:end].decode(ENCODING_JVDATA, errors='replace')
