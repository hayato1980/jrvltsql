...
# H1（票数１・全掛式）は native/リアルタイム/標準名のすべてが同じ公式スナップショット
# を保持する。消去・値域検証・schema 検証はその全部を対象にする。
_H1_NATIVE_STORAGE_TABLES = frozenset({"NL_H1", "RT_H1"})
_H1_STANDARD_OWNER_TABLE = "HYOSU"
_H1_STANDARD_CHILD_TABLES = frozenset(
    {
        "HYOSU_TANPUKU",
        "HYOSU_WAKU",
        "HYOSU_UMARENWIDE",
...
def _verify_hn_no_unapproved_constraints(
    database: BaseDatabase,
    table_name: str,
) -> None:
    """Reject constraints beyond HN's one official immediate primary key."""
    ...
    if targets != (database,):
        for target in targets:
            _verify_hn_no_unapproved_constraints(target, table_name)
        return
    ...
    if db_type == "sqlite":
        if database.fetch_all(f'PRAGMA foreign_key_list("{table_name}")'):
            raise SchemaMigrationError(
                f"HN storage {table_name} has unsupported FOREIGN KEY constraints"
            )
        row = database.fetch_one(
            "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ?",
            (table_name,),
        ...
        if re.search(r"\bCHECK\s*\(", definition, flags=re.IGNORECASE):
            raise SchemaMigrationError(f"HN storage {table_name} has unsupported CHECK constraints")
        return
    if db_type == "postgresql":
        unexpected = database.fetch_all(
            "SELECT conname AS constraint_name, contype AS constraint_type "
        ...
        if unexpected:
            raise SchemaMigrationError(
                f"HN storage {table_name} has unsupported additional constraints: {unexpected}"
            )
        return
    raise SchemaMigrationError(f"HN constraints cannot be verified for database type {db_type!r}")


def verify_hn_storage_schema(database: BaseDatabase, table_name: str) -> bool:
    """Fail closed unless HN storage preserves every field and official key."""

    if table_name not in _HN_STORAGE_TABLES:
        return False
    transaction_snapshot = _snapshot_validation_transactions(database)
    from src.database.migration import verify_table_schema
    from src.database.schema import SCHEMAS
    ...
    try:
        schema_sql = SCHEMAS.get(table_name) or JRAVAN_SCHEMAS.get(table_name)
        if schema_sql is None:
            raise SchemaMigrationError(f"HN storage schema is undefined: {table_name}")
        verify_table_schema(database, table_name, schema_sql)
        _verify_strict_storage_column_contract(
            database,
            table_name,
            ...
            allow_missing_columns=False,
            storage_label="HN",
            lossless_text_widths=_HN_LOSSLESS_TEXT_WIDTHS[table_name],
            allow_extra_columns=False,
        )
        _verify_hn_no_unapproved_constraints(database, table_name)
        _verify_replacement_key_constraints(database, table_name, "HN storage")
        return True
    except Exception:
        _rollback_call_created_validation_transactions(
            transaction_snapshot,
            context="failed HN schema validation",
        )
        raise


def validate_hn_record(record: dict, table_name: str | None = None) -> bool:
    """Validate one caller-built HN row and its standard aliases."""

    if table_name is not None and table_name not in _HN_STORAGE_TABLES:
        return False
    if _record_type_from_record(record) != "HN":
        if table_name is None:
            return False
        raise SchemaMigrationError(f"{table_name} received a non-HN record")
...
def _verify_h1_no_unapproved_constraints(
    database: BaseDatabase,
    table_name: str,
) -> None:
    """Reject CHECK/FOREIGN KEY constraints beyond the official H1 key."""
    ...
    if targets != (database,):
        for target in targets:
            _verify_h1_no_unapproved_constraints(target, table_name)
        return
    ...
    if db_type == "sqlite":
        if database.fetch_all(f'PRAGMA foreign_key_list("{table_name}")'):
            raise SchemaMigrationError(
                f"H1 storage {table_name} has unsupported FOREIGN KEY constraints"
            )
        row = database.fetch_one(
            "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ?",
            (table_name,),
        ...
        definition = _sqlite_schema_code(str((row or {}).get("sql") or ""))
        if re.search(r"\bCHECK\s*\(", definition, flags=re.IGNORECASE):
            raise SchemaMigrationError(f"H1 storage {table_name} has unsupported CHECK constraints")
        return
    if db_type == "postgresql":
        unexpected = database.fetch_all(
            "SELECT conname AS constraint_name, contype AS constraint_type "
        ...
        if unexpected:
            raise SchemaMigrationError(
                f"H1 storage {table_name} has unsupported CHECK/FOREIGN KEY constraints"
            )
        return
    raise SchemaMigrationError(f"H1 constraints cannot be verified for database type {db_type!r}")


def _h1_official_key_columns(table_name: str) -> tuple[str, ...]:
    """The official key that one H1 replacement is allowed to match."""

    if table_name == _H1_STANDARD_OWNER_TABLE:
        return _STANDARD_VOTE_RACE_KEY_COLUMNS
    return tuple(_STANDARD_VOTE_CONFIG["H1"]["children"][table_name])


def _verify_h1_standard_replacement_key(
    database: BaseDatabase,
    table_name: str,
) -> None:
    """Reject UNIQUEs that a standard-schema H1 replacement does not control.

    The standard vote family is keyed by the official race key (plus 馬番/組番 for
    the children) and the importer maintains exactly that unique index. Any other
    UNIQUE/exclusion index would make one race's snapshot replace or reject a
    different race's rows.
    """
    ...
    if targets != (database,):
        for target in targets:
            _verify_h1_standard_replacement_key(target, table_name)
        return

    official = {column.lower() for column in _h1_official_key_columns(table_name)}
    db_type = database.get_db_type()
    if db_type == "sqlite":
        unexpected = []
        ...
        if unexpected:
            raise SchemaMigrationError(
                f"H1 storage {table_name} has unsupported additional UNIQUE "
    ...
    if db_type == "postgresql":
        ...
        if unexpected:
            raise SchemaMigrationError(
                f"H1 storage {table_name} has unsupported additional UNIQUE/exclusion "
...
def validate_h1_record(record: dict, table_name: str | None = None) -> bool:
    """Validate one expanded H1 row against the official 28,955-byte domain.

    Native ``NL_H1``/``RT_H1`` and the standard ``HYOSU`` family all store the
    same official spans, so the official values are validated before any
    schema-specific expansion or coercion runs.
    """

    if table_name is not None and table_name not in _H1_STORAGE_TABLES:
        return False
    if _record_type_from_record(record) != "H1":
        if table_name is None:
            return False
        raise SchemaMigrationError(f"{table_name} received a non-H1 record")

    from src.parser.h1_parser import H1Parser

    try:
        data_kubun = resolve_record_data_kubun(record)
        H1Parser.validate_current_fields(record, data_kubun=data_kubun)
    except ValueError as error:
        raise SchemaMigrationError(str(error)) from error
    return True


def verify_h1_storage_schema(database: BaseDatabase, table_name: str) -> bool:
    """Fail closed unless H1 storage keeps the official snapshot and race key.

    Native ``NL_H1``, realtime ``RT_H1`` and the standard ``HYOSU`` owner/child
    tables all replace one complete race snapshot at a time. A missing table, a
    key that no longer distinguishes 賭式/組番, an additional UNIQUE/exclusion
    constraint, or a PostgreSQL primary key that ``ON CONFLICT`` cannot use would
    let one replacement erase or duplicate another race. A nullable key,
    wrong-typed, under-capacity, generated, extra, or additionally constrained
    column would lose official values, including the non-numeric 人気順 markers.
    All are rejected before any DML. Verifying the owner table also verifies its
    children, because one physical record spans all of them.
    """

    if table_name not in _H1_STORAGE_TABLES:
        return False
    transaction_snapshot = _snapshot_validation_transactions(database)
    from src.database.migration import verify_table_schema
    from src.database.schema import SCHEMAS
    ...
    try:
        if table_name == _H1_STANDARD_OWNER_TABLE:
            targets = (table_name, *sorted(_H1_STANDARD_CHILD_TABLES))
        else:
            targets = (table_name,)
        for target_table in targets:
            schema_sql = SCHEMAS.get(target_table) or JRAVAN_SCHEMAS.get(target_table)
            if schema_sql is None:
                raise SchemaMigrationError(f"H1 storage schema is undefined: {target_table}")
            ...
            _verify_h1_no_unapproved_constraints(database, target_table)
            if target_table in _H1_NATIVE_STORAGE_TABLES:
                _verify_replacement_key_constraints(database, target_table, "H1 storage")
            else:
                _verify_h1_standard_replacement_key(database, target_table)
        return True
    except Exception:
        _rollback_call_created_validation_transactions(
            transaction_snapshot,
            context="failed H1 schema validation",
        )
        raise
